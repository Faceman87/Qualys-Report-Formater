...
'===============================================
'Copy header to sheet 4,3 and 2,1
'===============================================
...
Worksheets.Add(After:=Sheets(Sheets.Count)).Name = "2,1"
...
ActiveSheet.Range(filterString).AutoFilter Field:=11, Criteria1:="=1", _
...
Selection.RowHeight = 15
Range("A1").Select
ActiveWorkbook.Worksheets("2,1").Sort.SortFields.Add Key:=Range(SortSeverity), _
        SortOn:=xlSortOnValues, Order:=xlDescending, DataOption:=xlSortNormal
    With ActiveWorkbook.Worksheets("2,1").Sort
        .SetRange Range(SortAll)
        .Header = xlYes
        .MatchCase = False
        .Orientation = xlTopToBottom
